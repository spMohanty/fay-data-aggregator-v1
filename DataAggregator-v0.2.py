...
def resample_cgm_for_user(user_df: pd.DataFrame) -> pd.DataFrame:
    ...
    user_df["gap_indicator"] = (
        user_df["read_at"].diff() > GLUCOSE_TIMESERIES_GAP_SPLIT_THRESHOLD
    ).cumsum()
    ...
    for _, block_subset in user_df.groupby("gap_indicator"):
        # Handle duplicates in read_at
        dup_mask = block_subset["read_at"].duplicated(keep=False)
        if dup_mask.any():
            total_duplicate_count += dup_mask.sum()
        block_subset = block_subset.drop_duplicates(subset=["read_at"])

        # Resample to 15-min
        block_subset = block_subset.set_index("read_at").asfreq(RESAMPLE_INTERVAL)
